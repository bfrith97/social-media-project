...
<div class="d-md-flex align-items-center mb-4">
    ...
    <div class="avatar me-3 mb-3 mb-md-0">
        <a href="{{ route('profiles.show', $otherUser->id) }}">
            <img class="avatar-img rounded-circle" src="{{ $otherUser->profile_picture ? asset($otherUser->profile_picture) : '' }}" alt="">
        </a>
    </div>
    <!-- Info -->
    <div class="w-100">
        <div class="d-sm-flex align-items-start">
            ...
            <p class="small ms-sm-2 mb-0">{{ $otherUser->role }}</p>
        </div>
        <!-- Connections START -->
        <ul class="avatar-group mt-1 list-unstyled align-items-sm-center">
            <li class="small">
                {{ $section }}
                for {{ Carbon\Carbon::parse($otherUser->pivot->created_at)->timezone('Europe/London')->longAbsoluteDiffForHumans() }}
            </li>
        </ul>
        <!-- Connections END -->
    </div>
